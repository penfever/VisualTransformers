...
# print(device)
N_TOKENS = 8
BATCH_SIZE_TRAIN = BATCH_SIZE_TEST = 400
...
if DISTRIBUTED:
    ...
    BS_FSL = N_TOKENS
DL_PATH = "/scratch/bf996/omniglot_merge/" # Use your own path
SUBSET_SIZE = 100
MODEL_DIM = 256
...
test_loader = torch.utils.data.DataLoader(test_dataset, batch_size=BATCH_SIZE_TEST,
                                         shuffle=False, num_workers=DEV_CT*8)

# holdout_loader = torch.utils.data.DataLoader(omniglot, batch_size=BATCH_SIZE_TRAIN, sampler=torch.utils.data.SubsetRandomSampler(indices[:2]))
holdout_loader = torch.utils.data.DataLoader(holdout_subset, batch_size=N_TOKENS, shuffle=True, num_workers=DEV_CT*8)
...
def fsl_eval(model, data_loader, criterion, scheduler):
    ...
    with torch.no_grad():
        for data, target in data_loader:
            # print("entering fsl: data size {}".format(data.size()))
            # print("entering fsl: target size {}".format(target.size()))
            # print("batch size {}".format(BS_FSL))
...
with wandb.init(project="RN34-SeqTrans-Omniglot", config=config):
    ...
    for epoch in range(1, N_EPOCHS + 1):
        ...
        if epoch % 500 == 1:
            ...
            PATH = "./ViTRes_{}_{}.pt".format(d4, current_time) # Use your own path
