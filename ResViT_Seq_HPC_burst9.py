...
if pr[0] > BURSTY:
    ...
    assert(np.equal(np.sum(weights), 1))
train_sampler = torch.utils.data.WeightedRandomSampler(weights, train_dataset, replacement=True, num_samples=SAMPLING_SIZE)
test_sampler = torch.utils.data.WeightedRandomSampler(test_dataset, replacement=True, num_samples=SAMPLING_SIZE)
holdout_sampler = torch.utils.data.WeightedRandomSampler(holdout_subset, replacement=True, num_samples=SAMPLING_SIZE)
...
with wandb.init(project="RN34-SeqTrans-Omniglot-Burst9", config=config):
    LATEST = "./ViTRes_Latest_{}.pt".format(STRBURST)
    ...
    if Path(LATEST).exists():
        ...
        EPOCH_RES = int(epoch_file[0].stem)
        print("Resuming from epoch {}".format(EPOCH_RES))
        START = EPOCH_RES
        if EPOCH_RES > 500000:
            raise("500k epochs reached, breaking")
        #Adjust target learning rate depending on whre we are in the longer training regimen
        LR = LR * (1 - (abs(250000 - EPOCH_RES) / 250000))
        assert(0 < LR < 3e-4)
    warmup = 250 if EPOCH_RES > 1000 else 4000
    ...
    for epoch in range(START, N_EPOCHS + START):
        ...
        if epoch % 500 == 1:
            ...
            strepoch = str(epoch)
            PATH = "./ViTRes_{}_{}_burst_{}_epoch_{}.pt".format(d4, current_time, STRBURST, strepoch) # Use your own path
