...
class seqTrans(nn.Module):
    def __init__(self, label_embedding, all_labels, num_classes=10, dim = 64, mlp_dim = 256, heads = 8, depth = 12, emb_dropout = 0.1, dropout= 0.1):
        ...
        self.positional_encoder = PositionalEncoding(
            dim_model=dim, dropout_p=dropout, max_len=50
        )
...
BATCH_SIZE_TRAIN = BATCH_SIZE_TEST = SCALER * N_TOKENS * DEV_CT
...
holdout_loader = torch.utils.data.DataLoader(omniglot_holdout, batch_size=N_TOKENS, num_workers=WORKERS, sampler=torch.utils.data.RandomSampler(omniglot_holdout, replacement=True, num_samples=SAMPLING_SIZE * 5 // SCALER), drop_last=True)
...
test_sampler = torch.utils.data.RandomSampler(test_dataset, replacement=True, num_samples=SAMPLING_SIZE * 5 // SCALER)
...
# MAIN LOOP
with wandb.init(project="{}-{}-burst{}".format(config['architecture'], config['dataset'], BURSTY), config=config, settings=wandb.Settings(start_method="fork")):
    ...
    if DISTRIBUTED and DEV_CT > 1:
        print("Using {} GPUs".format(DEV_CT))
