...
#         d_k = self.att_size # hidden_size / num_heads
#         d_v = self.att_size
#         batch_size = q.size(0)
...
#         print("after matmul: ")
#         print("x = {}, mask = {}".format(x.size(), mask.size()))
...
class EncoderLayer(nn.Module):
    def __init__(self, hidden_size, filter_size, dropout_rate):
        ...
        # MultiheadAttention(embed_dim=dim, num_heads = heads, dropout = dropout, batch_first=True)
        self.self_attention = MultiheadAttention(embed_dim=hidden_size, num_heads = 8, dropout = dropout_rate, batch_first=True)
...
class DecoderLayer(nn.Module):
    def __init__(self, hidden_size, filter_size, dropout_rate):
        ...
        self.enc_dec_attention = MultiheadAttention(embed_dim=hidden_size, num_heads = 8, dropout = dropout_rate, batch_first=True)
    ...
    def forward(self, x, enc_output, self_mask, i_mask, cache):
        ...
        if enc_output is not None:
            ...
            y = self.enc_dec_attention(y, enc_output, enc_output, None,
                                       cache)
            y = self.enc_dec_attention_dropout(y[0])
            x = x + y
...
class Transformer(nn.Module):
    ...
    def encode(self, inputs, i_mask):
        ...
        i_mask = i_mask.squeeze(-2)
        # print(inputs.size(), i_mask.size())
        input_embedded.masked_fill_(i_mask, 0)
    ...
    def decode(self, targets, enc_output, i_mask, t_self_mask, t_mask,
               cache=None):
        # target embedding
        target_embedded = targets
        t_mask = t_mask.squeeze(-2)
        # print(inputs.size(), i_mask.size())
        target_embedded.masked_fill_(t_mask, 0)
